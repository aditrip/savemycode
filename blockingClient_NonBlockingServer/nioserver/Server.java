...
public class Server {
					...
					for (SelectionKey scKey : selectedKeys) {
						...
						case SelectionKey.OP_READ:
							if (scKey.channel() instanceof SocketChannel) {
								RequestTask task = new RequestTask(scKey);
	...
	class RequestTask implements Runnable {
		// This is a read and write task on a blocking socket.

		private SocketChannel channel;
		private SelectionKey scKey;

		public RequestTask(SelectionKey scKey) throws IOException {
			this.scKey = scKey;
			...
			while (true) {
				try {
					...
					if (requestLine.equals("CLOSE")) {
						out.println("Got CLOSE Request, server is closing this socket:"
								+ channel.socket());
