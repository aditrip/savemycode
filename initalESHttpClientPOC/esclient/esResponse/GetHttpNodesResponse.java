 ...
 * information. This API is used in Sniffer used by ESHttpClient to determine
 * current available nodes with published http/https port.
 * 
 * A note on ES 2.x and ES 5.x changes: * in ES 2.x http_address was available.
 * in ES 5.x there is a publish_address inside a structure named http. ES 2.x
 * also has this publish_address. And this will be used as the host address.
 *
 *
 */
public class GetHttpNodesResponse extends ESResponse implements
        JsonResponseObjectMapper<GetHttpNodesResponse> {
     ...
     * @return
     */
    public List<HttpHost> getHttpHosts(Scheme scheme) {

        List<HttpHost> httpHosts = new ArrayList<HttpHost>();
        for (HttpNode httpNode : httpNodes) {
            String publishAddress = httpNode.getHttpInfo()
                                            .getPublish_address();
            URI uri = URI.create(scheme.getProtocol() + "://" + publishAddress);
    ...
    public static class HttpNode {
        ...
        public HttpNode buildFromJson(JsonParser parser) throws IOException {
            ...
            JsonUtil.validateToken(JsonToken.START_OBJECT, parser.nextToken(),
                                   parser);
            while ((token = parser.nextToken()) != JsonToken.END_OBJECT) {
                if (token == JsonToken.FIELD_NAME) {
                    currentFieldName = parser.getCurrentName();
