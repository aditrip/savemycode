...
public interface ESRestRequestGenerator {
    ...
    static class RequestParams {
        ...
        RequestParams refreshType(String refreshType) {
            if (!refreshType.toLowerCase().equals("false")) {
                return putParam("refresh", refreshType.toLowerCase());
            }
            return this;
        }
        ...
        RequestParams retryOnConflict(int retryOnConflict) {
            if (retryOnConflict > 0) {
                return putParam("retry_on_conflict",
                                String.valueOf(retryOnConflict));
            }
            return this;
        }
        ...
        RequestParams queryString(String queryString) {
            ...
            putParam("q", StringEscapeUtils.escapeHtml(queryString));
            return this;
        }

        RequestParams sort(String sort) {
            if (sort != null && sort.length() > 0) {
                putParam("sort", StringEscapeUtils.escapeHtml(sort));
            }
            return this;
        }
        ...
        RequestParams pageSize(int pageSize) {
            if (pageSize > 0) {
                putParam("size", Integer.toString(pageSize));
            }
            return this;
        }
        ...
        RequestParams fromIndex(int fromIndex) {
            if (fromIndex > 0) {
                putParam("from", Integer.toString(fromIndex));
            }
            return this;
        }
        
        /*
         * Values for level can be:
         * cluster,indices or shards.
         * Only cluster level is supported in Response.
         */
