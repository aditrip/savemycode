...
public interface JsonResponseObjectMapper<T> {
	...
	/**
	 * This method is called to build a response object from the
	 * Json Response in the HttpEntity sent by the ES Server.
	 * 
	 ...
	 * For eg, if the structure this method will build is the top level structure
	 * object, the parser needs to be typically positioned before the
	 * START_OBJECT, for inner structure, the parser will typically be 
	 * positioned at the JsonToken.START_OBJECT 
	 * and the currentFieldName is the structure name which it will build.
	 * 
	 * 
	 *
	 * 
	 * @param parser  - JsonParser positioned correctly at the start
	 ...
	 */
	 ...
	 /**
	  * Some Response Object may need to get built even in case
	 ...
	 /**
	  * Some Response types need not be parsed and can be used directly.
	  * For eg GetMappingResponse only contains the mapping and need not
	  * be parsed for more details.
	  * 
	  * @param restResp
	  * @return
	  * @throws IOException
