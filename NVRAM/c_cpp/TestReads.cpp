...
int main(int argc, char* argv[]) {
	...
	printf("\n Relevant VM Stats Before BufferGet");
	vmstat();
	...
	for (int j = 0; j < nBuffers ; j++) {
	 memcpy(dataBuffer, mmaps[j], bufferSize);
//         printf("databuffer[20]=%c",dataBuffer[20]);
	 }

	clock_gettime(CLOCK_REALTIME, &spec);
	end_time = spec;

	printf("\n\n **** MappingGET time:%ld microseconds ****",
			(diff(start_time, end_time).tv_nsec) / 1000);

	printf("\n Relevant VM Stats AFTER BUFFER GET");
	vmstat();
	
        // READ AGAIN IF PAGES WERE FAULTED IN
        clock_gettime(CLOCK_REALTIME, &spec);
	start_time = spec;
...
//         printf("databuffer[20]=%c",dataBuffer[20]);
	...
	printf("\n\n **** MappingGET AFTER PAGES RE FAULTED IN time:%ld microseconds ****",
			(diff(start_time, end_time).tv_nsec) / 1000);

	printf("\n Relevant VM Stats AFTER SECOND BUFFER GET");
	vmstat();
